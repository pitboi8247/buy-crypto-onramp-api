...
export const MoonPayTestWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  if (MoonPayEvent.status === 'completed') {
    fs.appendFile(filePath, MoonPayEvent.walletAddress.toLowerCase() + '\n', 'utf-8', (err) => {
      if (err) {
        console.error('Error adding address:', err.message);
        return console.log({ error: 'Error adding address' });
      }
...
export const MercuryoTestWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  if (mercuryoSignature) {
    ...
    if (mercuryoSignature === expectedSignature) {
      ...
      let pushResult = '';
      if (overideStatus === 'complete' || overideStatus === 'failed' || overideStatus === 'pending')
        pushResult = await sendBuyCryptoNotification(extractedData);
