...
export const MoonPayTestWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  if (moonpaySignatureV2Header) {
    ...
    if (signature === expectedSignature) {
      ...
      fs.appendFile('./addressess.txt', data.walletAddress + '\n', 'utf-8', err => {
				if (err) {
...
export const MercuryoTestWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  if (mercuryoSignature) {
    ...
    if (mercuryoSignature === expectedSignature) {
      ...
      let pushResult = '';
      if (status !== 'new') pushResult = await sendBuyCryptoNotification(extractedData);
      if (status === 'paid' || status === 'order_failed') {
        nodeNotifier.notify({
          title: 'Buy Crypto Alert',
          message: 'you have recieved an update for you crypto purchase',
        });
      }
      res.send({ 'webhook event verified': pushResult });
    } else {
