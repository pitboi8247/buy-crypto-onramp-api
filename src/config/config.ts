...
const envsSchema = z.object({
  ...
  MOONPAY_TEST_SECRET_KEY: z.string({ required_error: "moonpay test secret key required for url signing"}).nonempty(),
  MOONPAY_SECRET_KEY: z.string({ required_error: "moonpay secret key required for url signing"}).nonempty(),
  MOONPAY_LIVE_KEY: z.string({ required_error: "moonapy live key required for url signing"}).nonempty(),
  MOONPAY_WEBHOOK_KEY: z.string({ required_error: "Mercuryo webhook key required for url signing"}).nonempty(),
  MERCURYO_SIGN_KEY: z.string({ required_error: "Mercuryo secret key required for url signing"}).nonempty(),
  TRANSAK_API_KEY: z.string({ required_error: "Transak api key required for url signing"}).nonempty(),
  TRANSAK_TEST_API_KEY: z.string({ required_error: "Transak test api key required for url signing"}).nonempty(),
}).nonstrict();
